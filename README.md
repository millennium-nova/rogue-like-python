...
### 1. 手続き型ダンジョン生成
ローグライクの核となるマップ生成には、BSP（二分空間分割法）を採用しました。
Geminiに対して「部屋が重ならないように分割し、それらを通路で繋ぐ」という要件を提示し、出力されたロジックをプロジェクトのクラス設計に合わせて統合しました。

### 2. 敵キャラクターの追跡AI
敵が壁を避けてプレイヤーを追尾する機能には、A*アルゴリズムを使用しました。
複雑な経路探索ロジックはCopilotに生成させ、パラメータ（索敵範囲や行動頻度）の調整を行うことで、ゲームとして成立するバランスに仕上げました。
